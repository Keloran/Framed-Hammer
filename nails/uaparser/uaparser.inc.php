...
//				}
	...
	private function loadData() {
		...
		if ($cacheIni['lastupdate'] < time() - $this->updateInterval || $cacheIni['lastupdatestatus'] != "0") { $this->downloadData(); }
	...
	private function downloadData() {
		if(ini_get('allow_url_fopen')) {
			$status = 1;
			if (file_exists($this->cacheDir.'/cache.ini')) { $cacheIni = parse_ini_file($this->cacheDir.'/cache.ini'); }
	...
	/**
	 * UASparser::SetCacheDir()
	 *
	 * @param mixed $cache_dir
	 * @return
	 */
	public function SetCacheDir($cache_dir) {
		if (!is_writable($cache_dir)) { die('ERROR: Cache dir('.$cache_dir.') is not writable'); }
