<?php //netteCache[01]000410a:2:{s:4:"time";s:21:"0.25314800 1333123708";s:9:"callbacks";a:2:{i:0;a:3:{i:0;a:2:{i:0;s:19:"Nette\Caching\Cache";i:1;s:9:"checkFile";}i:1;s:90:"C:\Program Files\WaterProof\PHPEdit\4.0.0.11350\Tools\ApiGen\templates\default\class.latte";i:2;i:1332496707;}i:1;a:3:{i:0;a:2:{i:0;s:19:"Nette\Caching\Cache";i:1;s:10:"checkConst";}i:1;s:25:"Nette\Framework::REVISION";i:2;s:28:"$WCREV$ released on $WCDATE$";}}}?><?php

// source file: C:\Program Files\WaterProof\PHPEdit\4.0.0.11350\Tools\ApiGen\templates\default\class.latte

?><?php
// prolog Nette\Latte\Macros\CoreMacros
list($_l, $_g) = Nette\Latte\Macros\CoreMacros::initRuntime($template, 'eox77sfk2y')
;
// prolog Nette\Latte\Macros\UIMacros
//
// block title
//
if (!function_exists($_l->blocks['title'][] = '_lb17e9867ef1_title')) { function _lb17e9867ef1_title($_l, $_args) { extract($_args)
...
// block content
//
if (!function_exists($_l->blocks['content'][] = '_lbb71ed060dc_content')) { function _lbb71ed060dc_content($_l, $_args) { extract($_args)
...
<?php $iterations = 0; foreach ($iterator = $_l->its[] = new Nette\Iterators\CachingIterator($ownMethods) as $method): ?>
	<tr data-order="<?php echo htmlSpecialChars($method->name) ?>" id="_<?php echo htmlSpecialChars($method->name) ?>">
...
private<?php else: ?>public<?php endif ?> <?php if ($method->static): ?>static<?php endif ?>
		...
		<td class="name"><div>
		<a class="anchor" href="#_<?php echo htmlSpecialChars($method->name) ?>">#</a>
		<code><?php ob_start() ?>
...
<?php endif ;$iterations = 0; foreach ($iterator = $_l->its[] = new Nette\Iterators\CachingIterator($method->parameters) as $parameter): ?>
				<span><?php if ($parameter->array): ?>array
				<?php elseif ($parameter->className): echo $template->typeLinks($parameter->className, $method) ?>

				<?php elseif (isset($annotations['param'][$parameter->position])): echo $template->typeLinks($annotations['param'][$parameter->position], $parameter) ;endif ?>
...
</var><?php if ($parameter->defaultValueAvailable): ?> = <?php echo $template->highlightPHP($parameter->defaultValueDefinition, $class) ;endif ?>
</span><?php if (!$iterator->isLast()): ?>, <?php endif ?>
...
<?php $iterations = 0; foreach ($method->parameters as $parameter): ?>
					<dt><var>$<?php echo Nette\Templating\Helpers::escapeHtml($parameter->name, ENT_NOQUOTES) ?></var></dt>
					<dd><?php if (isset($annotations['param'][$parameter->position])): echo $template->annotation($annotations['param'][$parameter->position], 'param', $parameter) ;endif ?></dd>
...
<?php try { $prototype = $method->prototype ;if ($prototype->declaringClass->interface): ?>
				<h4>Implementation of</h4>
				<div class="list"><code><?php if ($_l->ifs[] = ($template->getClass($prototype->declaringClassName))): ?>
<a href="<?php echo htmlSpecialChars($template->methodUrl($prototype)) ?>"><?php endif ;echo Nette\Templating\Helpers::escapeHtml($prototype->declaringClassName, ENT_NOQUOTES) ?>
::<?php echo Nette\Templating\Helpers::escapeHtml($prototype->name, ENT_NOQUOTES) ;if (array_pop($_l->ifs)): ?>
</a><?php endif ?>
</code></div>
...
<?php $annotations = $constant->annotations ?>
		...
		<td class="attributes"><code>
			<?php if (isset($annotations['var'])): echo $template->typeLinks($annotations['var'][0], $constant) ;else: try { echo Nette\Templating\Helpers::escapeHtml($template->type($constant->value), ENT_NOQUOTES) ;} catch (\Exception $e) {} endif ?>
...
<?php $iterations = 0; foreach ($ownProperties as $property): ?>	<tr data-order="<?php echo htmlSpecialChars($property->name) ?>
" id="$<?php echo htmlSpecialChars($property->name) ?>">
<?php $annotations = $property->annotations ?>
...
private<?php else: ?>public<?php endif ?> <?php if ($property->static): ?>static<?php endif ?>
			...
			<?php if (isset($annotations['var'])): echo $template->typeLinks($annotations['var'][0], $property) ;else: try { echo Nette\Templating\Helpers::escapeHtml($template->type($property->defaultValue), ENT_NOQUOTES) ;} catch (\Exception $e) {} endif ?>
...
<?php endif ;endif ?>
		...
		<td class="value"><code><?php echo $template->highlightValue($property->defaultValueDefinition, $class) ?></code></td>
		<td class="description"><div>
			<a href="#$<?php echo htmlSpecialChars($property->name) ?>" class="anchor">#</a>
...
// block children
//
if (!function_exists($_l->blocks['children'][] = '_lb13a0d4966a_children')) { function _lb13a0d4966a_children($_l, $_args) { extract($_args)
